...
public class GameActivity extends AppCompatActivity implements SurfaceHolder.Callback2, Listener,
     ...
     * This can be null, usually if you override `onCreateSurfaceView` to render on the whole
     * activity window.
     */
    ...
    protected boolean processMotionEvent(MotionEvent event) {
        ...
        return onTouchEventNative(nativeHandle, event, event.getPointerCount(),
                event.getHistorySize(), event.getDeviceId(), event.getSource(), event.getAction(),
                event.getEventTime(), event.getDownTime(), event.getFlags(), event.getMetaState(),
                action, event.getButtonState(), cls, event.getEdgeFlags(), event.getXPrecision(),
                event.getYPrecision());
    }
    ...
    public void onGlobalLayout() {
        ...
        if (mLocation[0] != mLastContentX || mLocation[1] != mLastContentY || w != mLastContentWidth
            ...
            onContentRectChangedNative(nativeHandle, mLastContentX, mLastContentY,
     ...
     * @return the pointer to the C `GameActivity` struct associated to this activity or 0 if there
     *         is
     *     none.
     */
    ...
    protected void onCreateSurfaceView() {
        ...
        mSurfaceView.getHolder().addCallback(
                this); // Register as a callback for the rendering of the surface, so that we can
                       // pass this surface to the native code
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nativeHandle = initializeNativeCode(getAbsolutePath(getFilesDir()),
                getAbsolutePath(getObbDir()), getAbsolutePath(extDir), getAssets(),
