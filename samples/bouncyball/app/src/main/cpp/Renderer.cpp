...
void Renderer::start() {
    ...
    mHotPocketThread.run(
            [this](HotPocketState* hotPocketState) { hotPocketState->isStarted = true; });
    spin();
}
...
void Renderer::draw(ThreadState* threadState) {
    ...
    if (mSwappyEnabled) SwappyGL_recordFrameStart(threadState->display, threadState->surface);
