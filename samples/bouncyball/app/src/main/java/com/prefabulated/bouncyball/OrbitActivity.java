...
public class OrbitActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (start_intent.hasExtra("TEST_SCRIPT")) {
            ...
            Log.i(LOG_TAG,
                    String.format("Intent test script (%d) %s", test_data.length(), test_data));
        ...
        sfOffsetView.setText(String.format(
                Locale.US, "SF Offset: %.1f ms", sfVsyncOffsetNanos / (float) ONE_MS_IN_NS));
    ...
    public void doFrame(long frameTimeNanos) {
        ...
        swapIntervalView.setText(String.format(Locale.US, "Swap rate: %.2f Hz (%.2f ms)",
                1e9f / swapIntervalNS, swapIntervalNS * 1e-6f));
        ...
        mArrivalBinsLastSecond[Math.min(
                deltaToBin(arrivalDelta), mArrivalBinsLastSecond.length - 1)] += 1;
