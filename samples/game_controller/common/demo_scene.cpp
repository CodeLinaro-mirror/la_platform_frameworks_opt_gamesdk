...
void GameControllerCallback(const int32_t controllerIndex, const Paddleboat_ControllerStatus status,
                            void* userData) {
    if (userData != nullptr) {
...
void MotionDataCallback(const int32_t controllerIndex, const Paddleboat_Motion_Data* motionData,
                        void* userData) {
    if (userData != nullptr) {
        DemoScene* scene = reinterpret_cast<DemoScene*>(userData);
...
void DemoScene::RenderIntegratedMotionData() {
    ...
    const bool hasGyro = ((mIntegratedSensorFlags & PADDLEBOAT_INTEGRATED_SENSOR_GYROSCOPE) != 0);
    ;
    if (hasAccel) {
...
void DemoScene::RenderPanel_ControlsTab(const int32_t controllerIndex,
    ...
    if (layout == PADDLEBOAT_CONTROLLER_LAYOUT_ARCADE_STICK) {
        RenderPanel_ControlsTab_ArcadeStick(controllerIndex, controllerData, controllerInfo);
        return;
    }
    ...
    if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_VIRTUAL_MOUSE) != 0) {
        ...
        if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_TOUCHPAD) != 0) {
            bool touchpadDown = ((controllerData.buttonsDown & PADDLEBOAT_BUTTON_TOUCHPAD) != 0);
...
void DemoScene::RenderPanel_ControlsTab_ArcadeStick(
        const int32_t /*controllerIndex*/, const Paddleboat_Controller_Data& controllerData,
        const Paddleboat_Controller_Info& controllerInfo) {
    ...
    for (uint32_t index = UIBUTTON_A; index < UIBUTTON_COUNT; ++index) {
        ...
        buttonInfo.buttonState =
                (buttonsDown & buttonInfo.buttonMask) ? UIBUTTON_STATE_ACTIVE : UIBUTTON_STATE_IDLE;
        if (buttonInfo.enabled) {
            ControllerUIUtil::Button(panelParams, buttonId, buttonInfo);
        }
    }
...
void DemoScene::RenderPanel_InfoTab(const int32_t /*controllerIndex*/,
    ...
    if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_BATTERY) != 0) {
        ...
        ImGui::Text("Battery %.1f%%, %s", controllerData.battery.batteryLevel * 100.0f,
                    batteryStatusString);
    } else {
...
void DemoScene::RenderPanel_VibrationTab(const int32_t controllerIndex,
    ...
    if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_VIBRATION) != 0) {
        ...
        if (ImGui::Button(" Vibrate ")) {
            Paddleboat_Vibration_Data vibrationData = {static_cast<int32_t>(leftMotorDuration),
...
void DemoScene::RenderPanel_MotionTab(const int32_t /*controllerIndex*/,
    ...
    RenderMotionTableData(motionData, mAccelerometerTimestampDelta, mGyroscopeTimestampDelta,
                          hasAccel, hasGyro);
}
...
void DemoScene::RenderPanel_LightsTab(const int32_t controllerIndex,
    ...
    if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_LIGHT_PLAYER) != 0) {
        ...
        ImGui::DragInt("Player index", &playerIndex, 1.0f, 1, 4, "%d", ImGuiSliderFlags_NoInput);
    ...
    if (controllerIndex == PADDLEBOAT_INTEGRATED_SENSOR_INDEX) {
        if (motionData->motionType == PADDLEBOAT_MOTION_ACCELEROMETER) {
            UpdateMotionData(motionData, &mPreviousIntegratedAccelerometerTimestamp,
                             &mIntegratedAccelerometerTimestampDelta, mIntegratedAccelerometerData);
            ...
            UpdateMotionData(motionData, &mPreviousIntegratedGyroscopeTimestamp,
                             &mIntegratedGyroscopeTimestampDelta, mIntegratedGyroscopeData);
        }
        ...
        if (motionData->motionType == PADDLEBOAT_MOTION_ACCELEROMETER) {
            ...
            UpdateMotionData(motionData, &mPreviousGyroscopeTimestamp, &mGyroscopeTimestampDelta,
                             mGyroscopeData);
        }
    }
}
...
void DemoScene::OnInstall() {
    ...
    Paddleboat_setMotionDataCallbackWithIntegratedFlags(MotionDataCallback, mIntegratedSensorFlags,
                                                        this);
