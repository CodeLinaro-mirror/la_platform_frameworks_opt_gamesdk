...
const ControllerStickUIDefinition stickDefinition = {"gamecontroller/Thumbstick_Active.png",
    ...
    if (!uiDataInitialized) {
        for (int index = 0; index < UIBUTTON_COUNT; ++index) {
            buttonTextures[index].textureHandles[UIBUTTON_STATE_ACTIVE] =
                    TextureAssetLoader::loadTextureAsset(buttonDefinitions[index].assetName_Active,
            ...
            buttonTextures[index].textureHandles[UIBUTTON_STATE_IDLE] =
                    TextureAssetLoader::loadTextureAsset(buttonDefinitions[index].assetName_Idle,
                                                         nullptr, nullptr);
        }
        ...
        stickTextures.textureHandles[UISTICK_STATE_DEPRESSED] =
                TextureAssetLoader::loadTextureAsset(stickDefinition.assetName_Depressed, nullptr,
                                                     nullptr);
        stickTextures.textureHandles[UISTICK_STATE_IDLE] =
                TextureAssetLoader::loadTextureAsset(stickDefinition.assetName_Idle, nullptr,
                                                     nullptr);

        stickRegionTexture.textureHandles[0] =
                TextureAssetLoader::loadTextureAsset(stickRegionAssetName,
                                                     &stickRegionTexture.textureWidth,
