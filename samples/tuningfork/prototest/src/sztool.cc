...
size_t FieldSize(const FieldDescriptor* fd, bool outputTypes) {
    switch (fd->type()) {
        case FieldDescriptor::TYPE_BOOL:
            if (outputTypes) std::cout << fd->name() << " " << fd->type_name() << std::endl;
            // Do we want this?
            return 2;
        case FieldDescriptor::TYPE_ENUM:
            if (outputTypes) std::cout << fd->name() << " ";
            dynamicproto::print(fd->enum_type(), std::cerr);
            if (outputTypes) std::cerr << std::endl;
...
string RandomId() {
    const int ID_LEN = 32;
    char id[ID_LEN];
    for (int i = 0; i < ID_LEN - 1; ++i) id[i] = 32 + (RandInt32() % 90);
    id[ID_LEN - 1] = 0;
    return id;
}
...
Annotation MakeAnnotation(const FileDescriptor* fdesc,
    ...
    if (m) {
        ...
        for (int i = 0; i < ann_extensions.size(); ++i) {
            ...
            if (fd->type() == FieldDescriptor::TYPE_BOOL) {
                ...
                if (ann_values[i] > 0) r->SetEnumValue(m, fd, ann_values[i]);
            }
        }
        std::string s;
        ...
        std::cerr << "Warning, could not find " << ANNOTATION_MESSAGE_NAME << " in "
                  << fdesc->name() << std::endl;
    return a;
}

FidelityParams RandomFidelityParams(const FileDescriptor* fdesc,
    ...
    if (m) {
        ...
        for (int i = 0; i < fpExtensions.size(); ++i) {
            ...
            switch (fd->type()) {
                case FieldDescriptor::TYPE_BOOL:
                    ...
                    break;
                case FieldDescriptor::TYPE_ENUM:
                    r->SetEnumValue(m, fd, 1 + (RandInt32() % fd->enum_type()->value_count()));
                    break;
        ...
        std::cerr << "Warning, could not find " << FIDELITYPARAMS_MESSAGE_NAME << " in "
                  << fdesc->name() << std::endl;
    return p;
}
...
int main(int argc, char* argv[]) {
    ...
    dynamicproto::extensionsOf(fdescs.back(), "com.google.tuningfork.Annotation", ann_extensions);
    ...
    std::transform(ann_extensions.begin(), ann_extensions.end(), ann_extensionsSize.begin(),
    ...
    size_t nAnnotationCombinations =
            std::accumulate(ann_extensionsSize.begin(), ann_extensionsSize.end(), 1,
                            std::multiplies<size_t>());
    std::cerr << "Total number of annotation combinations = " << nAnnotationCombinations
              << std::endl;

    // FidelityParams extensions
    std::vector<const FieldDescriptor*> fpExtensions;
    dynamicproto::extensionsOf(fdescs.back(), "com.google.tuningfork.FidelityParams", fpExtensions);
    ...
    *ev.mutable_fidelityparams() = RandomFidelityParams(fdescs.front(), fpExtensions);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int c = 0; c < nAnnotationCombinations; ++c) {
            ...
            *live_h->mutable_annotation() =
                    MakeAnnotation(fdescs.front(), ann_extensions, ann_values);
            ...
            if (ZeroToOneRandomDouble() < fraction_of_annotations_explored) {
                for (int i = 0; i < h.n_buckets(); ++i) live_h->add_counts(RandInt32());
                nset++;
            }
        }
    }
    ...
    std::cout << "Wrote example clearcut message to " << outfname << " and " << dbgoutfname
              << std::endl;
    std::stringstream sout;
    ev.SerializeToOstream(&sout);
    std::cout << "Example message size = " << sout.str().length() << " bytes" << std::endl;
    return 0;
}
