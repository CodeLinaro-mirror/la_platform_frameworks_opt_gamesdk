...
void getMatrices(std::array<GLfloat, 16>& mvpMatrix, std::array<GLfloat, 9>& normalMatrix,
                 GLfloat aspectRatio, Vector3 pos, Quaternion rot) {
...
void Sphere::draw(float aspectRatio, const std::vector<Sphere>& spheres, int n_to_draw,
                  int workload) {
    ...
    // Positions
    glVertexAttribPointer(static_cast<GLuint>(state.vPositionHandle), 3, GL_FLOAT, GL_FALSE,
                          9 * sizeof(GLfloat), getVertices(workload).data());
    checkGlError("glVertexAttribPointer");
    glEnableVertexAttribArray(static_cast<GLuint>(state.vPositionHandle));
    checkGlError("glEnableVertexAttribArray");

    // Colors
    glVertexAttribPointer(static_cast<GLuint>(state.vColorHandle), 3, GL_FLOAT, GL_FALSE,
                          9 * sizeof(GLfloat), getVertices(workload).data() + 3);
    checkGlError("glVertexAttribPointer");
    glEnableVertexAttribArray(static_cast<GLuint>(state.vColorHandle));
    checkGlError("glEnableVertexAttribArray");

    // Normals
    glVertexAttribPointer(static_cast<GLuint>(state.vNormalHandle), 3, GL_FLOAT, GL_FALSE,
                          9 * sizeof(GLfloat), getVertices(workload).data() + 6);
    checkGlError("glVertexAttribPointer");
...
void update(Quaternion& rot, const Vector3& angular_velocity, float dt) {
    Quaternion q =
            {angular_velocity.x * rot.w + angular_velocity.y * rot.z - angular_velocity.z * rot.y,
