...
public:
    ApkValidator(const std::shared_ptr<android::ZipFileRO>& zip_file, const std::string& asset_path,
                 bool debug);
    int Validate(bool enforce_enums_in_annotations, bool check_dev_fidelity_params);

private:
    int ValidateAnnotation(const pb::Descriptor* desc, bool enforce_enums_in_annotations,
                           std::vector<int>& enum_sizes);
    int ValidateFidelityParams(const pb::Descriptor* desc);
    int ValidateSettings(const pb::Descriptor* sdesc, const std::vector<int>& def_ann_enum_size);
    int ValidateDevFidelityParams();
    const pb::Descriptor* FindMessageIgnoringScope(const pb::FileDescriptor* fdesc,
