...
void ErrorCollector::AddError(const std::string& file_name, int line, int column,
    ...
    std::cerr << "Error in: " << file_name << " line: " << line << " column: " << column
...
pb::io::ZeroCopyInputStream* ZipSourceTree::Open(const std::string& relative_path) {
    ...
    if (file_cache_.end() == it) {
        ...
        uint32_t file_len = 0;
        if (!zip_file_->getEntryInfo(entry, NULL, &file_len, NULL, NULL, NULL, NULL)) {
...
int ApkValidator::Validate(bool enforce_enums_in_annotations, bool check_dev_fidelity_params) {
    // Import the dev proto
    const pb::FileDescriptor* fdesc = importer_->Import(kDevProtoFilename);
    if (!fdesc) ERROR(source_tree_.GetLastErrorCode(), source_tree_.GetLastErrorMessage());
    ...
    auto aok = ValidateAnnotation(adesc, enforce_enums_in_annotations, def_ann_enum_size);
    ...
    if (!fpdesc) ERROR(ERROR_GETTING_FIDELITYPARAMS, "Error finding FidelityParams");
    ...
    if (!tfdesc) ERROR(source_tree_.GetLastErrorCode(), source_tree_.GetLastErrorMessage());
...
int ApkValidator::ValidateAnnotation(const pb::Descriptor* desc, bool enforce_enums_in_annotations,
...
int ApkValidator::ValidateFidelityParams(const pb::Descriptor* desc) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (field->type() == pb::FieldDescriptor::TYPE_GROUP ||
            ...
            ERROR(ERROR_FIDELITYPARAMS_BAD_TYPE, "FidelityParams can only contain scalars");
    }
    return NO_ERROR;
}
...
int ApkValidator::ValidateSettings(const pb::Descriptor* sdesc,
    ...
    if (!settings_proto) ERROR(ERROR_GETTING_SETTINGS, "Error making Settings prototype");
    ...
    std::unique_ptr<pb::io::ArrayInputStream> settings_bin(
            dynamic_cast<pb::io::ArrayInputStream*>(source_tree_.Open(kSettingsFilename)));
    if (!settings_bin) ERROR(ERROR_GETTING_SETTINGS, "Error reading tuningfork_settings.bin");
    ...
    auto aggregation_strategy_field = m->GetDescriptor()->FindFieldByName("aggregation_strategy");
    ...
    auto max_ikeys_field = agg_strat.GetDescriptor()->FindFieldByName("max_instrumentation_keys");
    auto max_ikeys = as_refl->GetInt32(agg_strat, max_ikeys_field);
    if (debug_) std::cout << "Max instrumentation keys = " << max_ikeys << std::endl;
    if (max_ikeys < 1 || max_ikeys > kMaxInstrumentationKeys)
        ERROR(ERROR_BAD_MAX_INSTRUMENTATION_KEYS, "max_ikeys = " << max_ikeys);
    auto ann_enum_size_field = agg_strat.GetDescriptor()->FindFieldByName("annotation_enum_size");
    std::vector<int> ann_enum_size;
    int n = as_refl->FieldSize(agg_strat, ann_enum_size_field);
    for (int i = 0; i < n; ++i)
        ann_enum_size.push_back(as_refl->GetRepeatedInt32(agg_strat, ann_enum_size_field, i));
    if (debug_) {
        std::cout << "Deduced annotation enum sizes from Annotation: " << def_ann_enum_size
                  << std::endl;
    ...
    for (int i = 0; i < ann_enum_size.size(); ++i) {
        if (def_ann_enum_size[i] != -1 && ann_enum_size[i] != def_ann_enum_size[i])
            ERROR(ERROR_BAD_ANNOTATION_ENUM_SIZE,
                  "Bad annotation enum size: " << ann_enum_size[i] << "!=" << def_ann_enum_size[i]);
    }
    return NO_ERROR;
}
...
int ApkValidator::ValidateDevFidelityParams() {
    ...
    int fp_count = 0;
    for (auto entry = zip_file->nextEntry(cookie); entry; entry = zip_file->nextEntry(cookie)) {
        ++fp_count;
    }
    zip_file->endIteration(cookie);
    if (debug_)
        std::cout << "Found " << fp_count << " files matching " << prefix + "*.bin" << std::endl;
    if (fp_count == 0) ERROR(ERROR_NO_DEV_FIDELITYPARAMS, "No dev fidelity params present");
    return NO_ERROR;
}

// Find a message type, ignoring the scope
const pb::Descriptor* ApkValidator::FindMessageIgnoringScope(const pb::FileDescriptor* fdesc,
...
void ApkValidator::DebugDesc(const pb::Descriptor* desc) {
    if (debug_) {
        std::cout << desc->name() << ": fields: " << desc->field_count()
                  ...
                  << ", extension ranges: " << desc->extension_range_count() << std::endl;
    }
}
