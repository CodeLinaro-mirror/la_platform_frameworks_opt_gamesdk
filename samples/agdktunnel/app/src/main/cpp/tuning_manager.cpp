...
typedef void (*func_AChoreographer_postFrameCallback64)(AChoreographer* choreographer,
...
void TuningManager::InitializeChoreographerCallback(AConfiguration* config) {
    ...
    if (sdkVersion >= 29) {
        ...
        AChoreographer_postFrameCallback(AChoreographer_getInstance(), choreographer_callback,
                                         this);
    }
#endif
}
...
void TuningManager::HandleChoreographerFrame() {
    ...
    if (pAChoreographer_postFrameCallback64 != nullptr) {
        pAChoreographer_postFrameCallback64(AChoreographer_getInstance(), choreographer_callback64,
                                            this);
    } else {
        AChoreographer_postFrameCallback(AChoreographer_getInstance(), choreographer_callback,
                                         this);
...
void TuningManager::StartLoading() {
    ...
    TuningFork_CProtobufSerialization cser =
            tuningfork::TuningFork_CProtobufSerialization_Alloc(annotation);
    startupLoadingMetadata.state = TuningFork_LoadingTimeMetadata::LoadingState::COLD_START;
    startupLoadingMetadata.network_latency_ns = 1234567;
    TuningFork_startRecordingLoadingTime(&startupLoadingMetadata,
                                         sizeof(TuningFork_LoadingTimeMetadata), &cser,
