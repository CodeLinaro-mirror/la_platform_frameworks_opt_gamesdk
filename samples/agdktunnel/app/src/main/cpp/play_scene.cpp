...
static const char* TONE_BONUS[] = {"d70 f150. f250. f350. f450.", "d70 f200. f300. f400. f500.",
...
void PlayScene::OnStartGraphics() {
    ...
    mTunnelGeom =
            new SimpleGeom(new VertexBuf(TUNNEL_GEOM, sizeof(TUNNEL_GEOM), TUNNEL_GEOM_STRIDE),
...
void PlayScene::DoFrame() {
    ...
    mFilteredSteerX =
            (mFilteredSteerX * (NOISE_FILTER_SAMPLES - 1) + mShipSteerX) / NOISE_FILTER_SAMPLES;
    mFilteredSteerZ =
            (mFilteredSteerZ * (NOISE_FILTER_SAMPLES - 1) + mShipSteerZ) / NOISE_FILTER_SAMPLES;
...
void PlayScene::RenderHUD() {
    ...
    NativeEngine::GetInstance()->GetMemoryConsumer()->RenderMemoryStatistics(mTextRenderer);
...
void PlayScene::UpdateProjectionMatrix() {
    ...
    mProjMat =
            glm::perspective(RENDER_FOV, mgr->GetScreenAspect(), RENDER_NEAR_CLIP, RENDER_FAR_CLIP);
}
