...
static const size_t ALLOC_SIZES[BUCKET_COUNT] = {(64 * 1024),   (128 * 1024), (256 * 1024),
                                                 (384 * 1024),  (512 * 1024), (768 * 1024),
                                                 (1024 * 1024), (2048 * 1024)};

// Milliseconds to delay before next alloc
static const uint64_t ALLOC_DELTA_TIME[BUCKET_COUNT] = {10, 20, 30, 40, 50, 75, 100, 200};
...
MemoryConsumer::MemoryConsumer(bool startAsActive) {
    ...
    MemoryAdvice_ErrorCode error =
            MemoryAdvice_registerWatcher(MEMORY_CALLBACK_PERIOD_MS, MemoryWatcherCallback, this);
...
void MemoryConsumer::UpdatePoolAllocations(const MemoryPools memoryPool,
    ...
    const uint64_t deltaTime = currentTimestamp - mPools[memoryPool]->GetLastAllocationTimestamp();
    if (deltaTime > mNextAllocationDeltaTime[memoryPool]) {
        ...
        if (allocPtr == NULL) {
            ALOGE("Failed to alloc %zu bytes for pool %d", allocSize, static_cast<int>(memoryPool));
        } else {
...
void MemoryConsumer::RenderMemoryStatistics(TextRenderer* textRenderer) const {
    if (mActive) {
        ...
        const size_t cachePoolMB = GetTotalPoolAllocatedBytes(MEMORY_POOL_CACHE) / (1024 * 1024);
        ...
        snprintf(memoryString, 63, "%s (%d) %d%% G: %zu MB C: %zu MB", statusString,
                 mCriticalMemoryWarningCount, percentAvailable, generalPoolMB, cachePoolMB);
