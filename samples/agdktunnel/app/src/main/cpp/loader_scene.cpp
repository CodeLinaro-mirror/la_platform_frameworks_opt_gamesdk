...
public:
    ...
    void FindTexturesFromAssetPack(const char* assetPackName) {
        ...
        const char** assetPackFiles =
                gameAssetManager->GetGameAssetPackFileList(assetPackName, &assetPackFileCount);
...
void LoaderScene::DoFrame() {
    ...
    if (gameAssetManager->GetGameAssetPackStatus(GameAssetManifest::EXPANSION_ASSETPACK_NAME) ==
        ...
        mTextureLoader->InstallTexturesFromAssetPack(GameAssetManifest::EXPANSION_ASSETPACK_NAME);
...
void LoaderScene::OnCreateWidgets() {
    ...
    mLoadingWidget = NewWidget()->SetText(S_LOADING)->SetTextColor(TEXT_COLOR)->SetCenter(TEXT_POS);
