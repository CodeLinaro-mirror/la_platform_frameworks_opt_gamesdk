...
public:
    ...
    void LoadGameAssetAsync(const char* assetName, const uint64_t bufferSize, void* loadBuffer,
                            LoadingCompleteCallback callback, AssetPackInfo* packInfo,
                            bool isInternal, void* userData);
...
void GameAssetManagerInternals::LoadGameAssetAsync(const char* assetName, const uint64_t bufferSize,
                                                   void* loadBuffer,
...
bool GameAssetManagerInternals::LoadExternalGameAsset(const char* assetName,
                                                      const uint64_t bufferSize, void* loadBuffer,
                                                      AssetPackInfo* packInfo) {
...
void GameAssetManagerInternals::UpdateAssetPackBecameAvailable(AssetPackInfo* assetPackInfo) {
    ...
    if (assetPackInfo->mAssetPackStatus != GameAssetManager::GAMEASSET_READY) {
        ...
        AssetPackErrorCode assetPackErrorCode =
                AssetPackManager_getAssetPackLocation(assetPackInfo->mDefinition->mPackName,
                                                      &assetPackLocation);
...
void GameAssetManagerInternals::SetAssetPackErrorStatus(const AssetPackErrorCode assetPackErrorCode,
    ...
    if (assetPackInfo == NULL) {
        ALOGE("%s failed with error code %d : %s", message, static_cast<int>(assetPackErrorCode),
        ...
        ALOGE("%s failed on asset pack %s with error code %d : %s", message,
              assetPackInfo->mDefinition->mPackName, static_cast<int>(assetPackErrorCode),
              mAssetPackErrorMessage);
    }
}
...
bool GameAssetManager::LoadGameAsset(const char* assetName, const size_t bufferSize,
                                     void* loadBuffer) {
    bool loadSuccess = false;

    if (assetName != NULL) {
#if defined NO_ASSET_PACKS
        loadSuccess = mInternals->LoadInternalGameAsset(assetName, bufferSize, loadBuffer);
#else
        AssetPackInfo* packInfo = mInternals->GetAssetPackForAssetName(assetName);
        if (packInfo != NULL) {
            if (packInfo->mAssetPackStatus == GameAssetManager::GAMEASSET_READY) {
...
bool GameAssetManager::LoadGameAssetAsync(const char* assetName, const size_t bufferSize,
...
const char** GameAssetManager::GetGameAssetPackFileList(const char* assetPackName,
                                                        int* fileListSize) {
