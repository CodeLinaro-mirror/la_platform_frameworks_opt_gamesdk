...
NativeEngine::NativeEngine(struct android_app* app) {
    ...
    jmethodID getInternalStoragePathID =
            GetJniEnv()->GetMethodID(activityClass, "getInternalStoragePath",
                                     "()Ljava/lang/String;");
    jobject jInternalStoragePath = GetJniEnv()->CallObjectMethod(mApp->activity->javaGameActivity,
                                                                 getInternalStoragePathID);
    jboolean isCopy;
    ...
    mCloudSaveEnabled = (bool)GetJniEnv()->CallBooleanMethod(mApp->activity->javaGameActivity,
                                                             isPlayGamesServicesLinkedID);
...
void NativeEngine::SaveGameToCloud(int level) {
    ...
    GetJniEnv()->CallVoidMethod(mApp->activity->javaGameActivity, saveCloudCheckpointID,
                                (jint)level);
}
...
static char sInsetsTypeName[][32] = {
        "CAPTION_BAR",     "DISPLAY_CUTOUT", "IME",         "MANDATORY_SYSTEM_GESTURES",
        "NAVIGATION_BARS", "STATUS_BARS",    "SYSTEM_BARS", "SYSTEM_GESTURES",
        "TAPABLE_ELEMENT", "WATERFALL",
};
...
void NativeEngine::HandleCommand(int32_t cmd) {
    ...
    switch (cmd) {
        ...
        case APP_CMD_INIT_WINDOW:
            ...
            VLOGD("HandleCommand(%d): hasWindow = %d, hasFocus = %d", cmd, mHasWindow ? 1 : 0,
                  mHasFocus ? 1 : 0);
            break;
        ...
        case APP_CMD_CONFIG_CHANGED:
            VLOGD("NativeEngine: %s",
                  cmd == APP_CMD_WINDOW_RESIZED ? "APP_CMD_WINDOW_RESIZED"
        ...
        case APP_CMD_WINDOW_INSETS_CHANGED:
            ...
            for (int type = 0; type < GAMECOMMON_INSETS_TYPE_COUNT; ++type) {
                ...
                VLOGD("%s insets: left=%d right=%d top=%d bottom=%d", sInsetsTypeName[type],
    ...
    VLOGD("NativeEngine: STATUS: F%d, V%d, W%d, EGL: D %p, S %p, CTX %p, CFG %p", mHasFocus,
          mIsVisible, mHasWindow, mEglDisplay, mEglSurface, mEglContext, mEglConfig);
}
...
bool NativeEngine::PrepareToRender() {
    if (mEglDisplay == EGL_NO_DISPLAY || mEglSurface == EGL_NO_SURFACE ||
        ...
        ALOGI("NativeEngine: binding surface and context (display %p, surface %p, "
              "context %p)",
...
void NativeEngine::DoFrame() {
    ...
    if (width != mSurfWidth || height != mSurfHeight) {
        ...
        ALOGI("NativeEngine: surface changed size %dx%d --> %dx%d", mSurfWidth, mSurfHeight, width,
              height);
        mSurfWidth = width;
        mSurfHeight = height;
...
void NativeEngine::SetInputSdkContext(int context) {
    ...
    GetJniEnv()->CallVoidMethod(mApp->activity->javaGameActivity, setInputContextID, (jint)context);
}
