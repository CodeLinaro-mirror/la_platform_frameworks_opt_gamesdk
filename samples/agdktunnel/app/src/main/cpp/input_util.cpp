...
static const InputAction _INPUT_ACTIONS[INPUT_ACTION_COUNT] =
        {{PADDLEBOAT_BUTTON_A, OURKEY_ENTER},        {PADDLEBOAT_BUTTON_B, OURKEY_ESCAPE},
         {PADDLEBOAT_BUTTON_DPAD_UP, OURKEY_UP},     {PADDLEBOAT_BUTTON_DPAD_LEFT, OURKEY_LEFT},
         {PADDLEBOAT_BUTTON_DPAD_DOWN, OURKEY_DOWN}, {PADDLEBOAT_BUTTON_DPAD_RIGHT, OURKEY_RIGHT}};
...
bool isDirectionalKey(const int32_t ourKeyCode) {
    return ourKeyCode == OURKEY_UP || ourKeyCode == OURKEY_LEFT || ourKeyCode == OURKEY_DOWN ||
...
bool CookGameActivityMotionEvent(GameActivityMotionEvent* motionEvent,
    ...
    if (motionEvent->pointerCount > 0) {
        ...
        if (pointerIndex != GAMEACTIVITY_MAX_NUM_POINTERS_IN_MOTION_EVENT) {
            callbackProcessed = _cookEventForPointerIndex(motionEvent, callback, ev, pointerIndex);
        }
    }
