...
enum TextureFileFormat { TEXTUREFILE_KTX = 0, TEXTUREFILE_ASTC, TEXTUREFILE_UNKNOWN };
...
static const uint8_t KTX_11_IDENTIFIER[KTX_IDENTIFIER_SIZE] = {0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31,
...
static bool CreateFromASTCFile(const uint8_t* fileData, const size_t fileSize, GLuint* textureID) {
    bool success = false;
    ...
    if (header->blockDepth == 1 && header->texDepth[0] == 1) {
        ...
        GLsizei textureHeight =
                header->texHeight[0] | (header->texHeight[1] << 8) | (header->texHeight[2] << 8);
        ...
        if (textureFormat > 0) {
            ...
            glCompressedTexImage2D(GL_TEXTURE_2D, 0, textureFormat, textureWidth, textureHeight, 0,
                                   textureSize, textureData);
...
static bool CreateFromKTXFile(const uint8_t* fileData, const size_t /*fileSize*/, GLuint* textureID,
    ...
    if (header->glInternalFormat >= GL_COMPRESSED_RGB8_ETC2 &&
        ...
        for (uint32_t currentMipLevel = 0; currentMipLevel < header->numberOfMipmapLevels;
            ...
            glCompressedTexImage2D(GL_TEXTURE_2D, currentMipLevel, header->glInternalFormat,
                                   currentMipWidth, currentMipHeight, 0, currentMipSize,
                                   textureData);
