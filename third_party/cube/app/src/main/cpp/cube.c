...
void DemoUpdateTargetIPD(struct demo* demo) {
    ...
    assert(!err);
    if (count) {
        past = (VkPastPresentationTimingGOOGLE*)malloc(sizeof(VkPastPresentationTimingGOOGLE) *
                                                       count);
        assert(past);
        ...
        for (uint32_t i = 0; i < count; i++) {
            if (!demo->syncd_with_actual_presents) {
                ...
                break;
            } else if (CanPresentEarlier(past[i].earliestPresentTime, past[i].actualPresentTime,
                ...
                late = false;
                demo->last_late_id = 0;
            } else if (ActualTimeLate(past[i].desiredPresentTime, past[i].actualPresentTime,
...
static void demo_draw(struct demo* demo) {
    ...
    if (demo->swappy_enabled) {
        SwappyVk_recordFrameStart(demo->present_queue, demo->swapchain, demo->current_buffer);
    }
...
static void demo_prepare_buffers(struct demo* demo) {
    ...
    assert(SwappyVk_initAndGetRefreshCycleDuration(env, demo->swappy_init_data.jactivity, demo->gpu,
...
static void demo_prepare_texture_image(struct demo* demo, const char* filename,
    ...
    if (required_props & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
        ...
        err = vkMapMemory(demo->device, tex_obj->mem, 0, tex_obj->mem_alloc.allocationSize, 0,
                          &data);
        assert(!err);
...
void demo_prepare_cube_data_buffers(struct demo* demo) {
    ...
    for (unsigned int i = 0; i < demo->swapchainImageCount; i++) {
        ...
        vkGetBufferMemoryRequirements(demo->device,
                                      demo->swapchain_image_resources[i].uniform_buffer, &mem_reqs);
...
static VkShaderModule demo_prepare_shader_module(struct demo* demo, const uint32_t* code,
                                                 size_t size) {
    ...
    if (demo->separate_present_queue) {
        ...
        for (uint32_t i = 0; i < demo->swapchainImageCount; i++) {
            err = vkAllocateCommandBuffers(demo->device, &present_cmd_info,
                                           &demo->swapchain_image_resources[i]
...
static void demo_cleanup(struct demo* demo) {
    ...
    if (!demo->is_minimized) {
        for (i = 0; i < demo->swapchainImageCount; i++) {
            vkDestroyFramebuffer(demo->device, demo->swapchain_image_resources[i].framebuffer,
                                 NULL);
        }
...
static void demo_create_xlib_window(struct demo* demo) {
    ...
    demo->xlib_window =
            XCreateWindow(demo->display, RootWindow(demo->display, vInfoTemplate.screen), 0, 0,
...
static void handle_ping(void* data UNUSED, struct wl_shell_surface* shell_surface,
                        uint32_t serial) {
    wl_shell_surface_pong(shell_surface, serial);
}

static void handle_configure(void* data UNUSED, struct wl_shell_surface* shell_surface UNUSED,
...
static const struct wl_shell_surface_listener shell_surface_listener = {handle_ping,
                                                                        handle_configure,
                                                                        handle_popup_done};
...
static VkResult demo_create_display_surface(struct demo* demo) {
    ...
    for (plane_index = 0; plane_index < plane_count; plane_index++) {
        ...
        err = vkGetDisplayPlaneSupportedDisplaysKHR(demo->gpu, plane_index, &supported_count,
                                                    supported_displays);
        assert(!err);
...
static struct SwappyVkFunctionProvider theVkFunctionProvider = {FunctionProviderInit,
...
static void demo_init_vk(struct demo* demo) {
    ...
    char* instance_validation_layers_alt2[] = {"VK_LAYER_GOOGLE_threading",
    ...
    if (demo->validate) {
        ...
        if (!validation_found) {
            ERR_EXIT("vkEnumerateInstanceLayerProperties failed to find required "
                     "validation layer.\n\n"
...
#elif defined(VK_USE_PLATFORM_IOS_MVK)
        ERR_EXIT("vkEnumerateInstanceExtensionProperties failed to find "
                 "the " VK_MVK_IOS_SURFACE_EXTENSION_NAME " extension.\n\n"
                 "Do you have a compatible Vulkan installable client driver (ICD) "
                 "installed?\n"
                 "Please look at the Getting Started guide for additional "
                 "information.\n",
...
#elif defined(VK_USE_PLATFORM_MACOS_MVK)
        ERR_EXIT("vkEnumerateInstanceExtensionProperties failed to find "
                 "the " VK_MVK_MACOS_SURFACE_EXTENSION_NAME " extension.\n\n"
                 "Do you have a compatible Vulkan installable client driver (ICD) "
                 "installed?\n"
                 "Please look at the Getting Started guide for additional "
                 "information.\n",
    ...
    if (demo->validate) {
        ...
        demo->CmdEndDebugUtilsLabelEXT =
                (PFN_vkCmdEndDebugUtilsLabelEXT)vkGetInstanceProcAddr(demo->inst,
...
static void registry_handle_global_remove(void* data UNUSED, struct wl_registry* registry UNUSED,
    ...
    if (argc > 0) {
        ...
        if (argv == NULL) {
            ...
            for (int iii = 0; iii < argc; iii++) {
                ...
                if (argv[iii] != NULL) {
                    wcstombs_s(&numConverted, argv[iii], wideCharLen + 1, commandLineArgs[iii],
                               wideCharLen + 1);
                }
            }
        }
