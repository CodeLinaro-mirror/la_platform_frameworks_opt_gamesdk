...
JSON11_TEST_CASE(json11_test) {
    ...
    auto json_failing_comment =
            Json::parse(failing_comment_test, err_failing_comment, JsonParse::COMMENTS);
...

    {
        ...
        for (const auto& tst : tests) {
            ...
            JSON11_TEST_ASSERT((size_t)std::count_if(res.begin(), res.end(), [](const Json& j) {
                                   return !j.is_null();
    ...
    JSON11_TEST_ASSERT(json_obj_str ==
