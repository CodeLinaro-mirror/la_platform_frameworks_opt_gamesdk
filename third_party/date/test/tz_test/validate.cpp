...
void test_info(const date::time_zone* zone, const date::sys_info& info) {
    ...
    if (end < sys_days{jan / 1 / 3000}) {
        ...
        if (nlocal < slocal + microseconds{1}) {
            ...
            try {
                local = slocal + microseconds{1} + (nlocal - (slocal + microseconds{1})) / 2;
                assert(false);
            } catch (const nonexistent_local_time&) {
            }
        }
...
void tzmain() {
    ...
    for (auto& zone : db.zones) names.push_back(zone.name());
#else  // !USE_OS_TZDB
    names.reserve(db.zones.size() + db.links.size());
    for (auto& zone : db.zones) names.push_back(zone.name());
    for (auto& link : db.links) names.push_back(link.name());
    ...
    for (auto const& name : names) {
        ...
        std::cout << "Initially:           ";
        if (info.offset >= seconds{0}) std::cout << '+';
        ...
        for (begin = info.end; begin < end; begin = info.end) {
            ...
            test_info(z, info);
            if (info.offset == prev_offset && info.abbrev == prev_abbrev && info.save == prev_save)
                continue;
