...
Java_com_google_androidgamesdk_GameSdkDeviceInfoJni_getProtoSerialized(JNIEnv* env, jobject) {
    ...
    size_t bufferSize = -1;
    if (pb_get_encoded_size(&bufferSize,
                            androidgamesdk_deviceinfo_GameSdkDeviceInfoWithErrors_fields, &proto)) {
        pb_byte_t* buffer = new pb_byte_t[bufferSize];
        pb_ostream_t stream = pb_ostream_from_buffer(buffer, bufferSize);
        if (pb_encode(&stream, androidgamesdk_deviceinfo_GameSdkDeviceInfoWithErrors_fields,
                      &proto)) {
