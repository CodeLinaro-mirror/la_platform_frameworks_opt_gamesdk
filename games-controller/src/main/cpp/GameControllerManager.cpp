...
void Java_com_google_android_games_paddleboat_GameControllerManager_onControllerConnected(
        JNIEnv* env, jobject gcmObject, jintArray deviceInfoArray, jfloatArray axisMinArray,
        jfloatArray axisMaxArray, jfloatArray axisFlatArray, jfloatArray axisFuzzArray) {
    ...
    if (deviceInfo != nullptr) {
        ...
        if ((infoArraySize * sizeof(int32_t)) ==
            ...
            ALOGE("deviceInfoArray/GameControllerDeviceInfo::InfoFields size "
                  "mismatch");
        }
        ...
        if ((axisArraySize * sizeof(float)) == (sizeof(float) * paddleboat::MAX_AXIS_COUNT)) {
            ...
            env->GetFloatArrayRegion(axisFlatArray, 0, axisArraySize, deviceInfo->getFlatArray());
            env->GetFloatArrayRegion(axisFuzzArray, 0, axisArraySize, deviceInfo->getFuzzArray());
        } else {
        ...
        jmethodID getDeviceNameById =
                env->GetMethodID(paddleboat::GameControllerManager::getGameControllerClass(),
                                 "getDeviceNameById", "(I)Ljava/lang/String;");
        if (getDeviceNameById != NULL) {
            jstring deviceNameJstring = reinterpret_cast<jstring>(
                    env->CallObjectMethod(paddleboat::GameControllerManager::
                                                  getGameControllerObject(),
...
void Java_com_google_android_games_paddleboat_GameControllerManager_onMotionData(
        JNIEnv* env, jobject gcmObject, jint deviceId, jint motionType, jlong timestamp,
        jfloat dataX, jfloat dataY, jfloat dataZ) {
    paddleboat::GameControllerManager::onMotionData(deviceId, motionType, timestamp, dataX, dataY,
                                                    dataZ);
}
...
constexpr const char* CLASSLOADER_LOADCLASS_METHOD_SIG = "(Ljava/lang/String;)Ljava/lang/Class;";

constexpr const char* GCM_CLASSNAME = "com/google/android/games/paddleboat/GameControllerManager";
...
constexpr const char* GCM_SETREPORTMOTIONEVENTS_METHOD_NAME = "setReportMotionEvents";
...
Paddleboat_ErrorCode GameControllerManager::init(JNIEnv* env, jobject jcontext) {
    ...
    jclass gcmClass = jclass(env->CallObjectMethod(classLoaderObject, loadClassID, gcmClassName));
    if (gcmClass == NULL) {
...
#endif
    jobject gcmObject =
            env->NewObject(global_gcmClass, gcm->mInitMethodId, jcontext, printControllerInfo);
    if (gcmObject == NULL) {
...
Paddleboat_ErrorCode GameControllerManager::initMethods(JNIEnv* env) {
    const MethodTableEntry methodTable[] =
            {{GCM_INIT_METHOD_NAME, GCM_INIT_METHOD_SIGNATURE, &mInitMethodId},
             {GCM_GETAPILEVEL_METHOD_NAME, GCM_GETAPILEVEL_METHOD_SIGNATURE, &mGetApiLevelMethodId},
             {GCM_GETBATTERYLEVEL_METHOD_NAME, GCM_GETBATTERYLEVEL_METHOD_SIGNATURE,
              &mGetBatteryLevelMethodId},
             {GCM_GETBATTERYSTATUS_METHOD_NAME, GCM_GETBATTERYSTATUS_METHOD_SIGNATURE,
              &mGetBatteryStatusMethodId},
             {GCM_GETINTEGRATED_METHOD_NAME, GCM_GETINTEGRATED_METHOD_SIGNATURE,
              &mGetIntegratedSensorMethodId},
             {GCM_SETACTIVESENSOR_METHOD_NAME, GCM_SETACTIVESENSOR_METHOD_SIGNATURE,
              &mSetActiveIntegratedSensorsMethodId},
             {GCM_SETVIBRATION_METHOD_NAME, GCM_SETVIBRATION_METHOD_SIGNATURE,
              &mSetVibrationMethodId},
             {GCM_SETLIGHT_METHOD_NAME, GCM_SETLIGHT_METHOD_SIGNATURE, &mSetLightMethodId},
             {GCM_SETNATIVEREADY_METHOD_NAME, VOID_METHOD_SIGNATURE, &mSetNativeReadyMethodId},
    ...
    for (size_t i = 0; i < methodTableCount; ++i) {
        ...
        jmethodID methodID =
                env->GetMethodID(mGameControllerClass, entry.methodName, entry.methodSignature);
        if (methodID == NULL) {
                        ...
                        if (gcm->mGameControllers[i].getControllerStatus() ==
                            ...
                            if (deviceInfo.getInfo().mDeviceId == eventDeviceId) {
                                ...
                                if (eventType == AINPUT_EVENT_TYPE_KEY) {
                                    handledEvent =
                                            gcm->processControllerKeyEvent(event,
                                                                           gcm->mGameControllers
                                                                                   [i]);
                                } else if (eventType == AINPUT_EVENT_TYPE_MOTION) {
                                    handledEvent =
                                            gcm->mGameControllers[i].processMotionEvent(event);
                                }
                                break;
                            }
                        }
    ...
    if (event != nullptr) {
        ...
        if (gcm) {
            ...
            const float* axisValues =
                    gcm->getAxisValuesFromGameActivityMotionEvent(event, eventSize, 0);
            if (axisValues != nullptr) {
                ...
                if (dpadSource == AINPUT_SOURCE_DPAD || gamepadSource == AINPUT_SOURCE_GAMEPAD ||
                    ...
                    handledEvent =
                            gcm->processGameActivityMouseEvent(event, eventSize, eventDeviceId);
                }
            }
        }
    }
            ...
            if (mGameControllers[i].getConnectionIndex() >= 0) {
                if (mGameControllers[i].getControllerStatus() == PADDLEBOAT_CONTROLLER_ACTIVE) {
                    ...
                    if (controllerInfo.deviceId == eventDeviceId) {
                        ...
                        const float axisP =
                                AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_PRESSURE, 0);
    ...
    if (mMouseStatus == PADDLEBOAT_MOUSE_PHYSICAL) {
        for (size_t i = 0; i < MAX_MOUSE_DEVICES; ++i) {
            if (mMouseDeviceIds[i] == eventDeviceId) {
                if (eventType == AINPUT_EVENT_TYPE_MOTION) {
                    mMouseData.mouseX = AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_X, 0);
                    mMouseData.mouseY = AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_Y, 0);
            ...
            if (mGameControllers[i].getConnectionIndex() >= 0) {
                if (mGameControllers[i].getControllerStatus() == PADDLEBOAT_CONTROLLER_ACTIVE) {
                    ...
                    if (controllerInfo.deviceId == eventDeviceId) {
                        Paddleboat_Controller_Data& controllerData =
                                mGameControllers[i].getControllerData();
                        controllerData.virtualPointer.pointerX = axisValues[AMOTION_EVENT_AXIS_X];
                        controllerData.virtualPointer.pointerY = axisValues[AMOTION_EVENT_AXIS_Y];
...
void GameControllerManager::update(JNIEnv* env) {
    ...
    if (!gcm->mGCMClassInitialized && gcm->mGameControllerObject != NULL) {
        gcm->mApiLevel = env->CallIntMethod(gcm->mGameControllerObject, gcm->mGetApiLevelMethodId);
        gcm->mIntegratedSensorFlags = static_cast<uint32_t>(
                env->CallIntMethod(gcm->mGameControllerObject, gcm->mGetIntegratedSensorMethodId));
    ...
    if (gcm->mSetNativeReadyMethodId != NULL) {
        env->CallVoidMethod(gcm->mGameControllerObject, gcm->mSetNativeReadyMethodId);
        gcm->mSetNativeReadyMethodId = NULL;
    }

    if (gcm->mMotionDataCallback != nullptr && gcm->mMotionEventReporting == false) {
        // If a motion data callback is registered, tell the managed side to
        // start reporting motion event data
        env->CallVoidMethod(gcm->mGameControllerObject, gcm->mSetReportMotionEventsMethodId);
    ...
    if (gcm->mActiveSensorFlagsDirty) {
        ...
        env->CallVoidMethod(gcm->mGameControllerObject, gcm->mSetActiveIntegratedSensorsMethodId,
            ...
            if (gcm->mGameControllers[i].getConnectionIndex() >= 0) {
                if (gcm->mGameControllers[i].getControllerStatus() ==
                    ...
                    if (mapData != nullptr) {
                        ...
                        ALOGI("No controller map found, using defaults for "
                              "vId/pId: %x %x",
                              gcm->mGameControllers[i].getDeviceInfo().getInfo()->mVendorId,
                              gcm->mGameControllers[i].getDeviceInfo().getInfo()->mProductId);
                    }
                    ...
                    gcm->mGameControllers[i]
                            .setupController(mapData, &gcm->mMappingInfo.mAxisTable[axisIndex],
                    ...
                    gcm->mGameControllers[i].setControllerStatus(PADDLEBOAT_CONTROLLER_INACTIVE);
...
bool GameControllerManager::isLightTypeSupported(const Paddleboat_Controller_Info& controllerInfo,
...
Paddleboat_ErrorCode GameControllerManager::setControllerLight(const int32_t controllerIndex,
                                                               const Paddleboat_LightType lightType,
    ...
    if (controllerIndex >= 0 && controllerIndex < PADDLEBOAT_MAX_CONTROLLERS) {
        GameControllerManager* gcm = getInstance();
        if (gcm) {
            if (gcm->mGameControllers[controllerIndex].getConnectionIndex() == controllerIndex) {
                const Paddleboat_Controller_Info& controllerInfo =
                        gcm->mGameControllers[controllerIndex].getControllerInfo();
                if (gcm->isLightTypeSupported(controllerInfo, lightType)) {
                    ...
                    env->CallVoidMethod(gcm->mGameControllerObject, gcm->mSetLightMethodId,
                    ...
                    if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_VIBRATION) !=
                        ...
                        if (gcm->mGameControllerObject != NULL &&
                            ...
                            env->CallVoidMethod(gcm->mGameControllerObject,
                                                gcm->mSetVibrationMethodId, controllerInfo.deviceId,
                                                intensityLeft, durationLeft, intensityRight,
                                                durationRight);
                        }
                    } else {
                        errorCode = PADDLEBOAT_ERROR_FEATURE_NOT_SUPPORTED;
    ...
    if (gcm) {
        ...
        for (size_t i = 0; i < PADDLEBOAT_MAX_CONTROLLERS; ++i) {
            if (gcm->mGameControllers[i].getConnectionIndex() < 0) {
                ...
                gcm->mGameControllers[i].setControllerStatus(PADDLEBOAT_CONTROLLER_JUST_CONNECTED);
#if defined LOG_INPUT_EVENTS
            ...
            if (gcm->mGameControllers[i].getConnectionIndex() >= 0) {
                ...
                if (deviceInfo.getInfo().mDeviceId == deviceId) {
                    gcm->mGameControllers[i].setControllerStatus(
                            PADDLEBOAT_CONTROLLER_JUST_DISCONNECTED);
#if defined LOG_INPUT_EVENTS
                    ALOGI("Setting PADDLEBOAT_CONTROLLER_JUST_DISCONNECTED on "
                          "index %d",
                          static_cast<int>(i));
#endif
                }
    ...
    if (mouseData != nullptr) {
        ...
        if (gcm) {
            if (gcm->mMouseStatus != PADDLEBOAT_MOUSE_NONE) {
                memcpy(mouseData, &gcm->mMouseData, sizeof(Paddleboat_Mouse_Data));
...
void GameControllerManager::onMotionData(const int32_t deviceId, const int32_t motionType,
    ...
    if (gcm) {
        if (gcm->mMotionDataCallback != nullptr) {
            ...
            motionData.motionType = static_cast<Paddleboat_Motion_Type>(motionType);
...
void GameControllerManager::setMouseStatusCallback(Paddleboat_MouseStatusCallback statusCallback,
                                                   void* userData) {
    GameControllerManager* gcm = getInstance();
    if (gcm) {
...
void GameControllerManager::onStop(JNIEnv* env) {
    ...
    if (gcm->mGameControllerObject != NULL) {
        jmethodID onPauseID = env->GetMethodID(gcm->mGameControllerClass, GCM_ONSTOP_METHOD_NAME,
...
void GameControllerManager::onStart(JNIEnv* env) {
    ...
    if (gcm->mGameControllerObject != NULL) {
        jmethodID onResumeID = env->GetMethodID(gcm->mGameControllerClass, GCM_ONSTART_METHOD_NAME,
                                                VOID_METHOD_SIGNATURE);
        if (onResumeID != NULL) {
        ...
        for (size_t i = 0; i < PADDLEBOAT_MAX_CONTROLLERS; ++i) {
            if (mGameControllers[i].getControllerStatus() == PADDLEBOAT_CONTROLLER_ACTIVE) {
                ...
                if ((controllerInfo.controllerFlags & PADDLEBOAT_CONTROLLER_FLAG_BATTERY) != 0) {
                    ...
                    jfloat batteryLevel = env->CallFloatMethod(mGameControllerObject,
                                                               mGetBatteryLevelMethodId, deviceId);
                    jint batteryStatus = env->CallIntMethod(mGameControllerObject,
                    ...
                    controllerData.battery.batteryStatus =
                            static_cast<Paddleboat_BatteryStatus>(batteryStatus - 1);
                }
            }
        }
    } else {
        ...
        if (fstat(fileDescriptor, &fileStat) == 0) {
            ...
            if (fileBuffer != nullptr) {
                if (read(fileDescriptor, fileBuffer, fileSize) == static_cast<ssize_t>(fileSize)) {
                    result = GameControllerManager::addControllerRemapDataFromFileBuffer(
                            addMode,
        ...
        if (result == PADDLEBOAT_NO_ERROR) {
            switch (addMode) {
                case PADDLEBOAT_REMAP_ADD_MODE_DEFAULT:
                    result = GameControllerMappingUtils::
                            mergeControllerRemapData(mappingFileHeader, mappingFileBufferSize,
                                                     gcm->mMappingInfo);
                    break;
                case PADDLEBOAT_REMAP_ADD_MODE_REPLACE_ALL:
                    result = GameControllerMappingUtils::
                            overwriteControllerRemapData(mappingFileHeader, mappingFileBufferSize,
                                                         gcm->mMappingInfo);
                    break;
            }
        }
...
void GameControllerManager::addControllerRemapData(
        const Paddleboat_Remap_Addition_Mode /*addMode*/, const int32_t /*remapTableEntryCount*/,
...
GameControllerManager::getMapForController(const GameController& gameController) {
    ...
    MappingTableSearch mapSearch(&mMappingInfo.mControllerTable[0],
                                 mMappingInfo.mControllerTableEntryCount);
    mapSearch.initSearchParameters(deviceInfo.getInfo().mVendorId, deviceInfo.getInfo().mProductId,
                                   mApiLevel, mApiLevel);
