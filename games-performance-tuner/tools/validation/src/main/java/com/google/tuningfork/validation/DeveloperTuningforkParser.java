...
public class DeveloperTuningforkParser {
    ...
    private void validateAndSaveBinarySettings(ImmutableList<Integer> enumSizes) {
        if (tuningforkSettings.isPresent()) {
            ...
            logger.atSevere().log(
                    "File %s exists: FAIL", FolderConfig.TUNINGFORK_SETTINGS_TEXTPROTO);
        }

        if (errors.hasAnnotationErrors()) {
            logger.atSevere().log("Skip %s file check as Annotation is not valid",
        ...
        if (errors.hasSettingsErrors() || !settings.isPresent()) {
            logger.atSevere().log("Skip saving %s file as %s contains errors",
                    FolderConfig.TUNINGFORK_SETTINGS_BINARY,
                    FolderConfig.TUNINGFORK_SETTINGS_TEXTPROTO);
        }
    ...
    private void encodeBinaryAndValidateDevFidelity(Descriptor fidelityField) {
        if (!devFidelityFiles.isEmpty()) {
            ...
            logger.atInfo().log("%d %s files found: OK\n%s", devFidelityFiles.size(),
                    FolderConfig.DEV_FIDELITY_TEXTPROTO, builder.toString());
        } else {
            logger.atSevere().log("%s files found: FAIL", FolderConfig.DEV_FIDELITY_TEXTPROTO);
        }

        if (errors.hasFidelityParamsErrors()) {
            logger.atSevere().log("Skip %s files check as FidelityParams message is not valid",
                    FolderConfig.DEV_FIDELITY_TEXTPROTO);
        }

        List<ByteString> filesContent = new ArrayList<>();
        devFidelityFiles.forEach(textprotoFile -> {
    ...
    private Optional<ByteString> encodeBinary(Descriptor fidelityField, File textprotoFile) {
        ...
        try {
            binaryFile = Optional.of(compiler.encodeFromTextprotoFile(fidelityField.getFullName(),
                    devTuningforkProto.get(), textprotoFile,
                    getBinaryPathForTextprotoPath(textprotoFile), Optional.empty()));
        } catch (IOException | CompilationException e) {
            errors.addError(ErrorType.DEV_FIDELITY_PARAMETERS_ENCODING,
                    String.format("Encoding %s file", textprotoFile.getName()), e);
        }
        Optional<ByteString> content = Optional.empty();
        try {
            content = Optional.of(ByteString.copyFrom(Files.toByteArray(binaryFile.get())));
        } catch (IOException e) {
            errors.addError(ErrorType.DEV_FIDELITY_PARAMETERS_READING,
                    String.format("Reading %s file", binaryFile.get().getName()), e);
        }
        return content;
    }

    private void validateDevFidelityParams(
            Descriptor fidelityField, List<ByteString> filesContent) {
        List<DynamicMessage> fidelityMessages =
                ValidationUtil.validateDevFidelityParams(filesContent, fidelityField, errors);
