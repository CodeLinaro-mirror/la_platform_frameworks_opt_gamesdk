...
final class ValidationUtil {
    ...
    private static final ImmutableList<FieldDescriptor.Type> ALLOWED_FIDELITYPARAMS_TYPES =
            ImmutableList.of(FieldDescriptor.Type.ENUM, FieldDescriptor.Type.FLOAT,
            ...
            if (histogram.hasBucketMax() && histogram.getBucketMax() < histogram.getBucketMin()) {
                errors.addWarning(ErrorType.HISTOGRAM_BUCKET_INVALID,
                        "Bucket_Min has to be less than Bucket_Max. Max currently is "
                                + histogram.getBucketMax() + "Min currently is "
                                + histogram.getBucketMin());
            }
    ...
    public static void validateSettingsAggregation(
        ...
        if (!aggregation.hasMaxInstrumentationKeys()) {
            errors.addError(ErrorType.AGGREGATION_INSTRUMENTATION_KEY,
        ...
        if (maxKey < 1 || maxKey > MAX_INTSTRUMENTATION_KEYS) {
            errors.addError(ErrorType.AGGREGATION_INSTRUMENTATION_KEY,
                    String.format("max_instrumentation_keys should be between 1 and %d, current "
                                    + "value is %d",
        ...
        if (annotationCount != enumSizes.size()) {
            errors.addError(ErrorType.AGGREGATION_ANNOTATIONS,
    ...
    private static final float getValueHelper(
            int i, List<DynamicMessage> fidelityMessages, FieldDescriptor field) {
     ...
     */
    public static final void validateDevFidelityParamsOrder(Descriptor fidelityParamsDesc,
            List<DynamicMessage> fidelityMessages, ErrorCollector errors) {
        long isIncreasing =
                fidelityParamsDesc.getFields()
                        .stream()
                        .filter(field
                                -> IntStream.range(1, fidelityMessages.size())
                                        .allMatch(i
                                                -> getValueHelper(i - 1, fidelityMessages, field)
                                                        <= getValueHelper(
                                                                i, fidelityMessages, field)))
                        .count();
        ...
        if (isIncreasing + isDecreasing - allEqual != fidelityParamsDesc.getFields().size()) {
            errors.addWarning(ErrorType.DEV_FIDELITY_PARAMETERS_ORDER,
                    "Fidelity parameters should be "
     ...
     */
    public static final void validateDevFidelityParamsZero(Descriptor fidelityParamsDesc,
            List<DynamicMessage> fidelityMessages, ErrorCollector errors) {
        Stream<FieldDescriptor> hasZeroEnum = fidelityParamsDesc.getFields().stream().filter(field
                -> fidelityMessages.stream()
                        .filter(message -> message.getField(field) instanceof EnumValueDescriptor)
                        .anyMatch(message
                                -> ((EnumValueDescriptor) message.getField(field)).getNumber()
                                        == 0));
    ...
    public static final List<DynamicMessage> validateDevFidelityParams(
            Collection<ByteString> devFidelityList, Descriptor fidelityParamsDesc,
            ErrorCollector errors) {
        if (devFidelityList.isEmpty()) {
            errors.addError(
                    ErrorType.DEV_FIDELITY_PARAMETERS_EMPTY, "Fidelity parameters list is empty");
            return null;
        }
        List<DynamicMessage> fidelityMessages = new ArrayList<>();
        devFidelityList.forEach(entry
                -> fidelityMessages.add(
                        validateDevFidelityParams(entry, fidelityParamsDesc, errors).get()));
    ...
    public static final Optional<DynamicMessage> validateDevFidelityParams(
        ...
        try {
            fidelityMessage =
                    Optional.of(DynamicMessage.parseFrom(fidelityParamsDesc, devFidelityContent));
            if (!fidelityMessage.isPresent()) {
                errors.addError(
                        ErrorType.DEV_FIDELITY_PARAMETERS_EMPTY, "Fidelity parameters is empty");
            }
        } catch (InvalidProtocolBufferException e) {
            errors.addError(ErrorType.DEV_FIDELITY_PARAMETERS_PARSING,
    ...
    public static final void validateFidelityParams(
        ...
        if (fidelityParamsDesc == null) {
            errors.addError(
                    ErrorType.FIDELITY_PARAMS_EMPTY, "FidelityParams descriptor is not exist");
            return;
        }

        if (!fidelityParamsDesc.getOneofs().isEmpty()) {
            errors.addError(ErrorType.FIDELITY_PARAMS_COMPLEX,
                    "FidelityParams too complex - has oneof field");
            return;
        }

        if (!fidelityParamsDesc.getNestedTypes().isEmpty()) {
            errors.addError(ErrorType.FIDELITY_PARAMS_COMPLEX,
                    "FidelityParams too complex - has nested types");
            return;
        }

        if (!fidelityParamsDesc.getExtensions().isEmpty()) {
            errors.addError(ErrorType.FIDELITY_PARAMS_COMPLEX,
                    "FidelityParams too complex - has extensions");
            return;
        }
        ...
        for (FieldDescriptor field : fidelityParamsDesc.getFields()) {
            ...
            if (!ALLOWED_FIDELITYPARAMS_TYPES.contains(type)) {
                errors.addError(ErrorType.FIDELITY_PARAMS_TYPE,
                        String.format("FidelityParams can only be of type FLOAT, INT32 or ENUM, %s "
                                        + "field has type %s",
                                field.getName(), type));
            }
        }
    ...
    public static final ImmutableList<Integer> validateAnnotationAndGetEnumSizes(
        ...
        if (!annotationDesc.getNestedTypes().isEmpty()) {
            errors.addError(
                    ErrorType.ANNOTATION_COMPLEX, "Annotation too complex - has nested types");
            return null;
        }
        ...
        for (FieldDescriptor field : annotationDesc.getFields()) {
            if (field.getType() != FieldDescriptor.Type.ENUM) {
                errors.addError(ErrorType.ANNOTATION_TYPE,
                        String.format("Annotation can only contains enums, but contains %s field "
                                        + "with type %s",
    ...
    private static final void validateSettingsLoadingAnnotationIndex(
        ...
        if (!settings.hasLoadingAnnotationIndex()) {
            errors.addWarning(
                    ErrorType.LOADING_ANNOTATION_INDEX_MISSING, "loading_annotation_index not set");
        }
    }
