...
public class UIValidator {
    ...
    public static void createTableValidator(
            Disposable disposable, JTable table, Supplier<ValidationInfo> validationMethod) {
        TableModelListener tableModelListener =
                new ComponentTableValidationUpdater(table, validationMethod);
        table.getModel().addTableModelListener(tableModelListener);
        Disposer.register(
                disposable, () -> table.getModel().removeTableModelListener(tableModelListener));
    ...
    public static void createSimpleValidation(Disposable disposable, JComponent jComponent,
        ...
        FocusListener focusListener =
                new ComponentFocusValidationUpdater(jComponent, validationMethod);
    ...
    public static void createComboBoxValidation(Disposable disposable, JComboBox<?> jComponent,
        ...
        ActionListener actionListener =
                new ComponentActionValidationUpdater(jComponent, validationMethod);
        jComponent.addActionListener(actionListener);
        Disposer.register(disposable, () -> jComponent.removeActionListener(actionListener));
    }

    public static void createSliderValidator(
            Disposable disposable, JSlider jSlider, Supplier<ValidationInfo> validationMethod) {
        ChangeListener changeListener =
                new ComponentSliderValidationUpdate(jSlider, validationMethod);
    ...
    public static void createTextValidator(Disposable disposable, JTextField jTextField,
        ...
        DocumentAdapter documentAdapter =
                new ComponentTextValidationUpdater(jTextField, validationMethod);
        jTextField.getDocument().addDocumentListener(documentAdapter);
        Disposer.register(
                disposable, () -> jTextField.getDocument().removeDocumentListener(documentAdapter));
    ...
    private static ValidationInfo hasValidationInfo(JComponent component) {
        return component != null
                ? (ValidationInfo) component.getClientProperty(CELL_VALIDATION_PROPERTY)
                : null;
    }

    private static Supplier<ValidationInfo> getValidationMethod(JComponent component) {
        return component != null
                ? (Supplier<ValidationInfo>) component.getClientProperty(VALIDATION_METHOD)
                : null;
    }
        ...
        for (int i = 0; i < jTable.getRowCount(); i++) {
            for (int j = 0; j < jTable.getColumnCount(); j++) {
                JComponent component =
                        (JComponent) jTable.getCellRenderer(i, j).getTableCellRendererComponent(
                                jTable, tableModel.getValueAt(i, j), false, false, i, 0);
            ...
            for (int j = 0; j < jTable.getColumnCount(); j++) {
                ...
                for (Component childComp : component.getComponents()) {
                    if (childComp instanceof JComponent
                            && !isComponentValid((JComponent) childComp)) {
                        return false;
                    }
                }
            }
    ...
    private static void updateValidationLineBorder(
            Supplier<ValidationInfo> validationMethod, JComponent jComponent) {
    ...
    private static void updateValidationRoundedBorder(
            Supplier<ValidationInfo> validationMethod, JTextField jComponent) {
        ValidationInfo cellInfo = validationMethod.get();
        jComponent.putClientProperty(CELL_VALIDATION_PROPERTY, cellInfo);
        if (cellInfo != null) {
            jComponent.setToolTipText(cellInfo.message);
    ...
    private final static class ComponentFocusValidationUpdater implements FocusListener {
        ...
        public ComponentFocusValidationUpdater(
                JComponent jComponent, Supplier<ValidationInfo> supplier) {
    ...
    private final static class ComponentActionValidationUpdater implements ActionListener {
        ...
        public ComponentActionValidationUpdater(
                JComponent jComponent, Supplier<ValidationInfo> supplier) {
            this.jComponent = jComponent;
            this.validationMethod = supplier;
        }
    ...
    private final static class ComponentTextValidationUpdater extends DocumentAdapter {
        ...
        public ComponentTextValidationUpdater(
                JTextField jComponent, Supplier<ValidationInfo> supplier) {
            this.jComponent = jComponent;
            this.validationMethod = supplier;
        }
    ...
    private final static class ComponentTableValidationUpdater implements TableModelListener {
        ...
        public ComponentTableValidationUpdater(
                JComponent jComponent, Supplier<ValidationInfo> supplier) {
            this.jComponent = jComponent;
            this.validationMethod = supplier;
        }
    ...
    private final static class ComponentSliderValidationUpdate implements ChangeListener {
        ...
        public ComponentSliderValidationUpdate(
                JSlider component, Supplier<ValidationInfo> supplier) {
