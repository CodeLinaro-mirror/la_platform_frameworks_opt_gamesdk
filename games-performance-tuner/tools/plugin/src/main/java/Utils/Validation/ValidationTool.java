...
public final class ValidationTool {
    ...
    private static List<String> getEnumIndexes(List<String> optionsToBeTransformed,
        ...
        return IntStream.range(0, qualityDataModels.size())
                .mapToObj(settingsNumber
                        -> IntStream.range(0, currentOptions.size())
                                .filter(optionsIndex
                                        -> currentOptions.get(optionsIndex)
                                                .equals(optionsToBeTransformed.get(settingsNumber)))
                                .boxed()
    ...
    private static ArrayList<String> transformEnumToIndexSingleRow(
            List<QualityDataModel> qualityDataModels, ArrayList<String> qualityParams,
        ...
        if (fidelityMessage.getEnumData(row).isPresent()) {
            ...
            transformed = (ArrayList<String>) getEnumIndexes(
                    qualityParams, qualityDataModels, currentEnum.getOptions());
        }
    ...
    public static boolean isRowValid(
            List<QualityDataModel> qualityDataModels, int row, FieldType fieldType) {
    ...
    public static boolean isIncreasingSingleField(
            List<QualityDataModel> qualityDataModels, MessageDataModel fidelityMessage, int row) {
        ArrayList<String> qualityParams = getQualityForOneField(qualityDataModels, row);
        ArrayList<String> transformedQualityParams = transformEnumToIndexSingleRow(
                qualityDataModels, qualityParams, fidelityMessage, row);

        return IntStream.range(0, qualityDataModels.size() - 1)
                .allMatch(setting
                        -> Float.parseFloat(transformedQualityParams.get(setting))
                                <= Float.parseFloat(transformedQualityParams.get(setting + 1)));
    }

    public static boolean isDecreasingSingleField(
            List<QualityDataModel> qualityDataModels, MessageDataModel fidelityMessage, int row) {
        ArrayList<String> qualityParams = getQualityForOneField(qualityDataModels, row);
        ArrayList<String> transformedQualityParams = transformEnumToIndexSingleRow(
                qualityDataModels, qualityParams, fidelityMessage, row);
