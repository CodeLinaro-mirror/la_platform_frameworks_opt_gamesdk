...
public final class DataModelTransformer {
    ...
    public DataModelTransformer(String projectPath, ProtoCompiler compiler)
        ...
        File assetsDir = new File(AssetsFinder.findAssets(projectPath).getAbsolutePath());
    ...
    public static Optional<MessageDataModel> transformToAnnotation(Descriptor desc) {
        ...
        for (FieldDescriptor fieldDescriptor : fieldDescriptors) {
            if (!annotationDataModel.addField(
                        fieldDescriptor.getName(), fieldDescriptor.getEnumType().getName())) {
    ...
    public static Optional<QualityDataModel> transformToQuality(
            DynamicMessage message, List<FieldDescriptor> fieldDescriptors) {
    ...
    public static List<DynamicMessage> convertByteStringToDynamicMessage(
        ...
        messages.forEach(byteMessage -> {
            try {
                dynamicMessages.add(DynamicMessage.parseFrom(
                        descriptor, Base64.getDecoder().decode(byteMessage)));
        ...
        if (qualityFiles.isPresent()) {
            ...
            for (File qualityFile : qualityFilesList) {
                ...
                DynamicMessage fidelityMessage =
                        compiler.decodeFromBinary(fidelityDesc, fileContent);
                qualityDataModelList.add(
                        transformToQuality(fidelityMessage, fidelityDesc.getFields()).get());
            }
        }
