...
public class RequestServer {
    ...
    private RequestServer() throws IOException {
        localNetworkHttpServer = HttpServer.create(
                new InetSocketAddress(InetAddress.getLocalHost().getHostAddress(), 9000), 0);
    ...
    private Optional<String> replaceExponentSubstrings(String jsonString) {
        ...
        while (matcher.find()) {
            ...
            matcher.appendReplacement(
                    stringBuffer, "\"duration\": " + bigDecimal.longValue() + "s");
        }
    ...
    private void handleDebugInfo(HttpExchange httpExchange) throws IOException {
        ...
        JsonObject jsonObject =
                (JsonObject) new JsonParser().parse(new InputStreamReader(inputStream));
    ...
    private void handleGenerateTuningParameters(HttpExchange httpExchange) throws IOException {
        ...
        if (!fidelityParam.equals(ByteString.EMPTY)) {
            String parsedString = Base64.getEncoder().encodeToString(fidelityParam.toByteArray());
            String reply = "{\"parameters\": {\"serializedFidelityParameters\": \"" + parsedString
                    + "\"}}";
