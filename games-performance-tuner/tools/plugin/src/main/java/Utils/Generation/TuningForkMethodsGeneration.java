...
public class TuningForkMethodsGeneration {
    ...
    public void generateTuningForkCode(
            MessageDataModel annotationData, MessageDataModel fidelityData) {
        PsiFile temporaryPsiFile = PsiFileFactory.getInstance(project).createFileFromText(
                OCLanguage.getInstance(), generateCode(annotationData, fidelityData));
        for (PsiElement psiElement : temporaryPsiFile.getChildren()) {
            ...
            if (addedElement.isPresent()) {
                if (psiElement instanceof OCCppLinkageSpecification) {
                    Optional<PsiElement> methodDefinition =
                            cppFileUtils.findMethodAsChildren(psiElement);
    ...
    private String generateAnnotationMessage(MessageDataModel messageDataModel) {
        if (messageDataModel.getMessageType().equals(Type.ANNOTATION)) {
            ...
            stringBuilder.append("extern \"C\" void ")
                    .append("Set")
                    .append(messageName)
                    .append("s("); // extern "C" void setAnnotations(
            for (int i = 0; i < fieldNames.size(); i++) {
                stringBuilder.append("com::google::tuningfork::")
            ...
            for (int i = 0; i < fieldTypes.size(); i++) {
                stringBuilder.append(messageName.toLowerCase())
                        .append(".set_")
                        .append(fieldNames.get(i))
                        .append("(")
                        .append(fieldNames.get(i))
    ...
    private String generateFidelityMessage(MessageDataModel messageDataModel) {
        if (messageDataModel.getMessageType().equals(Type.FIDELITY)) {
            ...
            stringBuilder.append("extern \"C\" void ")
                    .append("Set")
                    .append(messageName)
                    .append("("); // extern "C" void setFidelity(
            for (int i = 0; i < fieldNames.size(); i++) {
