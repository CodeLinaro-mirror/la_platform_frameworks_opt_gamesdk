...
public class AnnotationTab extends TabLayout implements PropertyChangeListener {
    ...
    private final JBLabel annotationLabel = new JBLabel(resourceLoader.get("annotation_settings"));
    private final JBLabel informationLabel = new JBLabel(resourceLoader.get("annotation_info"));
    ...
    private void initVariables() {
        ...
        decoratorPanel =
                ToolbarDecorator.createDecorator(annotationTable)
                        ...
                        .setRemoveAction(
                                it -> annotationController.removeRowAction(annotationTable))
    ...
    private void initColumnViews() {
        ...
        enumColumn.setCellEditor(
                new EnumComboBoxDecorator(new TableComboBox<>(), annotationController.getEnums()));
        // noinspection UnstableApiUsage
        enumColumn.setCellRenderer(TableRenderer.getRendererComboBoxWithValidation(
                new EnumComboBoxDecorator(new TableComboBox<>(), annotationController.getEnums()),
                (value, row, column) -> {
        ...
        // noinspection UnstableApiUsage
        nameColumn.setCellRenderer(TableRenderer.getRendererTextBoxWithValidation(
                new RoundedCornerRenderer(), (value, row, column) -> {
                    ...
                    if (Pattern.compile(ILLEGAL_TEXT_PATTERN).matcher(strVal).find()) {
                        return new ValidationInfo(String.format(
                                resourceLoader.get("field_illegal_character_error"), strVal));
                    }
                    return null;
                }));
    ...
    public boolean isViewValid() {
        return UIValidator.isComponentValid(annotationTable)
                & UIValidator.isTableCellsValid(annotationTable);
    }
    ...
    private void initValidators() {
        UIValidator.createTableValidator(disposable, annotationTable, () -> {
            ...
            boolean isNamesDuplicate = fieldNames.stream().anyMatch(
                    option -> Collections.frequency(fieldNames, option) > 1);
            if (isNamesDuplicate) {
