...
public class InstrumentationSettingsTab extends TabLayout implements PropertyChangeListener {
    ...
    private final JBLabel informationLabel =
            new JBLabel("<html> Choose your preferred settings for game analysis. <br>"
                    + "Histograms:</html>");
    private JTextField baseUrl;
    private JPanel radioButtonsPanel;

    public InstrumentationSettingsTab(
            InstrumentationSettingsTabController controller, Disposable disposable) {
    ...
    private void initValidators() {
        ...
        UIValidator.createTableValidator(disposable, instrumentationTable, () -> {
            ...
            for (int i = 0; i < histogramsCount; i++) {
                ...
                if (!histogramCoversRecommendedSettings(histogram)) {
                    return new ValidationInfo(
                            String.format(
                                    RESOURCE_LOADER.get("unrecommended_histogram_warning"), i + 1))
                            .asWarning();
                }
            }
            return null;
        ...
        UIValidator.createSliderValidator(disposable, intervalSlider, () -> {
            ...
            if (value == 0) {
                ...
                return new ValidationInfo(RESOURCE_LOADER.get("unrecommended_slider_warning"))
                        .asWarning();
            }
            return null;
        });
        UIValidator.createSimpleValidation(disposable, radioButtonsPanel, () -> {
            if (radioButtonGroup.getSelection() == null) {
                return new ValidationInfo(
                        RESOURCE_LOADER.get("aggregation_strategy_not_chosen_error"));
            }
            return null;
        });
    ...
    public int getSliderMinimumRecommendedValue() {
        if (radioButtonTimeBased.isSelected()) {
            ...
            // Minimum value just to avoid any warning/errors without selecting aggregation
            // strategy.
            return 0;
        }
    }
    ...
    private void initComponents() {
        ...
        InstrumentationSettingsTableModel model = new InstrumentationSettingsTableModel(controller);
        ...
        intervalSlider.addChangeListener(e -> sliderValueLabel.setText(getSliderValue()));
    ...
    public void initIntegerTextField(JTable table, int col, boolean canBeZero) {
        ...
        if (!canBeZero) {
            column.setCellRenderer(TableRenderer.getRendererTextBoxWithValidation(
                    new RoundedCornerRenderer(), new IntegerTextFieldValidator()));
        } else {
            column.setCellRenderer(TableRenderer.getRendererTextBoxWithValidation(
                    new RoundedCornerRenderer(), new NonNegativeTextFieldValidator()));
        }
    }
    ...
    public void initNumberTextField(JTable table, int col) {
        ...
        column.setCellEditor(getIntegerTextFieldModel());
        column.setCellRenderer(TableRenderer.getRendererTextBoxWithValidation(
                new RoundedCornerRenderer(), new NumberTextFieldValidator()));
    }
    ...
    private static final class IntegerTextFieldValidator implements TableCellValidator {
        ...
        public ValidationInfo validate(Object o, int i, int i1) {
            ...
            if (bigInteger.compareTo(BigInteger.ZERO) <= 0) {
                return new ValidationInfo(
                        String.format(RESOURCE_LOADER.get("less_than_zero_error"), str));
            }
            return null;
        }
    }
    ...
    private static final class NonNegativeTextFieldValidator implements TableCellValidator {
        ...
        public ValidationInfo validate(Object o, int i, int i1) {
            ...
            if (bigInteger.compareTo(BigInteger.ZERO) < 0) {
                return new ValidationInfo(
                        String.format(RESOURCE_LOADER.get("non_negative_error"), str));
            }
            return null;
        }
    }
