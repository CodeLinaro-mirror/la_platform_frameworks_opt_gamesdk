 ...
 * transfer being called twice. Where the user had to click twice on the same combobox to see the
 * popup So this combobox is a better version of the original combobox as it has error balloon and
 * validation And for table cell editor/renderer just extend the BaseComboBoxTableCell and override
 * the methods
 */
public class TableComboBox<T> extends JComboBox<T> {
    ...
    public static class BaseComboBoxTableCell
    ...
    private static final class CustomFocusBorderUI extends DarculaComboBoxUI {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
            ...
            if (holderBox.isAddFocusBorder()) {
                DarculaUIUtil.paintOutlineBorder(
                        g2, r.width, r.height, arc, true, true, Outline.focus);
            }
            g2.dispose();
        }
    }
