...
public class MonitoringFilterDialogWrapper extends DialogWrapper {
    ...
    protected void doOKAction() {
        if (!filterPanel.isViewValid()) {
            Messages.showErrorDialog(RESOURCE_LOADER.get("fix_errors_first"),
    ...
    private final class FilterPanel extends JPanel {
        ...
        private void initComponent() {
            ...
            annotationDecorator = ToolbarDecorator.createDecorator(annotationsTree)
                                          .addExtraAction(getAnnotationTreeButton())
                                          .createPanel();
        ...
        private Set<Node> getAllFidelity() {
            List<Node> chosenAnnotationNodes =
                    selectedAnnotationNodes.stream()
                            .map(jTreeNode -> histogramTree.findNodeByName(jTreeNode.getNodeName()))
                            .collect(Collectors.toList());
            return histogramTree.getAllFidelity(chosenAnnotationNodes);
        }

        private SelectAction getAnnotationTreeButton() {
            return new SelectAction(annotationsTree,
                    ()
                            -> annotationsTree.getSelectionPath() != null,
                    lastNodes -> lastNodes.forEach(lastNode -> {
                        Optional<JTreeNode> nodeExist =
                                selectedAnnotationNodes.stream()
                                        .filter(node
        ...
        private class SelectAction extends AnActionButton {
            ...
            public SelectAction(
                    JTree jTree, Supplier<Boolean> update, Consumer<List<JTreeNode>> consumer) {
            ...
            public void actionPerformed(@NotNull AnActionEvent anActionEvent) {
                ...
                List<JTreeNode> lastNodes =
                        Arrays.stream(jTree.getSelectionPaths())
                                .map(path -> (JTreeNode) path.getLastPathComponent())
                                .collect(Collectors.toList());
