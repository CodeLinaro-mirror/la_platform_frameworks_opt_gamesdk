...
public class EnumDialogWrapper extends DialogWrapper {
        ...
        if (isEdit) {
            if (controller.editEnum(editingRow, enumLayout.getName(), enumLayout.getOptions())) {
                ...
                Messages.showInfoMessage(
                        String.format(resourceLoader.get("enum_name_already_exist"),
                                enumLayout.getName()),
                        resourceLoader.get("unable_to_close_title"));
            }
        } else {
    ...
    private static final class EnumLayout extends TabLayout {
        ...
        public void setData(EnumDataModel enumDataModel) {
            ...
            enumDataModel.getOptions().forEach(
                    optionName -> model.addRow(new String[] {optionName}));
        }
        ...
        private void initComponents() {
            ...
            decoratorPanel = ToolbarDecorator.createDecorator(optionsTable)
        ...
        private void initValidators() {
            UIValidator.createTextValidator(disposable, nameTextField, () -> {
                ...
                if (name.isEmpty()) {
                    return new ValidationInfo(
                            resourceLoader.get("field_empty_error"), nameTextField);
                }
                if (Pattern.compile(ILLEGAL_TEXT_PATTERN).matcher(name).find()) {
                    return new ValidationInfo(
                            String.format(
                                    resourceLoader.get("field_illegal_character_error"), name),
                            nameTextField);
                }
                return null;
            });
            ...
            UIValidator.createTableValidator(disposable, optionsTable, () -> {
                if (optionsTable.getRowCount() == 0) {
                    return new ValidationInfo(
                            resourceLoader.get("options_table_empty_error"), optionsTable);
                }
                ArrayList<String> options = getOptions();
                boolean isOptionDuplicate = options.stream().anyMatch(
                        option -> Collections.frequency(options, option) > 1);
                if (isOptionDuplicate) {
                    return new ValidationInfo(
                            resourceLoader.get("repeated_fields_error"), optionsTable);
                }
                return null;
            });
            optionsTable.getColumnModel().getColumn(0).setCellEditor(TabLayout.getTextFieldModel());

            // noinspection UnstableApiUsage
            optionsTable.getColumnModel().getColumn(0).setCellRenderer(
                    TableRenderer.getRendererTextBoxWithValidation(
                            new RoundedCornerRenderer(), (value, row, column) -> {
                                String strVal = value.toString();
                                if (strVal.isEmpty()) {
                                    return new ValidationInfo(
                                            resourceLoader.get("field_empty_error"));
                                }
                                if (Pattern.compile(ILLEGAL_TEXT_PATTERN).matcher(strVal).find()) {
                                    return new ValidationInfo(String.format(
                                            resourceLoader.get("field_illegal_character_error"),
                                            strVal));
                                }
                                return null;
                            }));
