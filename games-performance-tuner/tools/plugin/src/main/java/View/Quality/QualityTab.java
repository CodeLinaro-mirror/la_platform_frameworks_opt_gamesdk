            ...
            public TableCellRenderer getCellRenderer(int row, int column) {
                if (column == 0) {
                    ...
                    if (qualityTabController.isEnum(row)) {
                        return new EnumOptionsDecorator(
                                qualityTabController.getEnumOptionsByIndex(row));
                    } else {
        ...
        decoratorPanel =
                ToolbarDecorator.createDecorator(qualityParametersTable)
                        .setAddAction(anActionButton
                                -> qualityTabController.addColumn(qualityParametersTable))
                        .setRemoveAction(anActionButton
                                -> qualityTabController.removeColumn(qualityParametersTable))
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        switch (evt.getPropertyName()) {
            ...
            case "typeChange":
                ...
                qualityTableModel.setRowValue(
                        index, qualityTabController.getDefaultValueByIndex(index));
                break;
            case "editOptions":
                OptionalInt currentIndex = qualityTabController.getFidelityRowByEnumName(
                        ((Object[]) evt.getNewValue())[1].toString());
                currentIndex.ifPresent(value
                        -> qualityTableModel.setRowValue(
                                value, qualityTabController.getDefaultValueByIndex(value)));
        }
    }
