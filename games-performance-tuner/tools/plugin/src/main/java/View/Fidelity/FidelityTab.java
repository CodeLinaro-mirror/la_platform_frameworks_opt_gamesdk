...
public class FidelityTab extends TabLayout implements PropertyChangeListener {
    ...
    private final static FidelityFieldComboBox TABLE_FIELD_CELL_RENDERER =
            new FidelityFieldComboBox(new TableComboBox<>(),
                    new FieldType[] {FieldType.INT32, FieldType.FLOAT, FieldType.ENUM});
    private final static FidelityFieldComboBox TABLE_FIELD_CELL_EDITOR =
            new FidelityFieldComboBox(new TableComboBox<>(),
                    new FieldType[] {FieldType.INT32, FieldType.FLOAT, FieldType.ENUM});
    ...
    private TableCellRenderer getCellRendererByValue(FidelityTableData data) {
        if (data.getFieldType().equals(FieldType.ENUM)) {
            return new FidelityCellPanelValidationRendererWrapper(
                    new FidelityTableDecorators.JPanelDecorator(
                            fidelityTabController.getEnumNames()))
                    .withCellValidator(new FidelityTextCellValidation());
        }
        return TableRenderer.getRendererTextBoxWithValidation(
                new FidelityTableDecorators.TextBoxRenderer(), new FidelityTextCellValidation());
    }

    private TableCellEditor getCellEditorByValue(FidelityTableData data) {
        if (data.getFieldType().equals(FieldType.ENUM)) {
            return new FidelityTableDecorators.JPanelDecorator(
                    fidelityTabController.getEnumNames());
        }
        ...
        UIValidator.createTableValidator(disposable, fidelityTable, () -> {
            ...
            if (isNamesDuplicate) {
                return new ValidationInfo(
                        resourceLoader.get("repeated_fields_error"), fidelityTable);
            }
