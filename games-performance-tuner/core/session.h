...
public:
    ...
    FrameTimeMetricData* CreateFrameTimeHistogram(MetricId id, const Settings::Histogram& settings);
...
private:
    ...
    FrameTimeMetricData* TakeFrameTimeData(MetricId id) {
        for (auto it = available_frame_time_data_.begin(); it != available_frame_time_data_.end();
             ++it) {
            auto p = *it;
