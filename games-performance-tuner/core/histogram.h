...
public:
    explicit Histogram(Sample start = 0, Sample end = 0,
                       int num_buckets_between = kDefaultNumBuckets, bool never_bucket = false);
    ...
    Sample BucketStart() const {
        return start_;
    }
    Sample BucketEnd() const {
        return end_;
    }
...
Histogram<Sample>::Histogram(Sample start, Sample end, int num_buckets_between, bool never_bucket)
    ...
    switch (mode_) {
        case Mode::HISTOGRAM:
            if (bucket_size_ <= 0) ALOGE("Histogram end needs to be larger than histogram begin");
            break;
