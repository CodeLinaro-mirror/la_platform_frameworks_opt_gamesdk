...
public:
    TuningForkImpl(const Settings& settings, IBackend* backend, ITimeProvider* time_provider,
                   IMemInfoProvider* memory_provider, IBatteryProvider* battery_provider,
    ...
    TuningFork_ErrorCode GetFidelityParameters(const ProtobufSerialization& defaultParams,
                                               ProtobufSerialization& fidelityParams,
                                               uint32_t timeout_ms);
    ...
    TuningFork_ErrorCode RecordLoadingTime(Duration duration, const LoadingTimeMetadata& metadata,
                                           const ProtobufSerialization& annotation,
                                           bool relativeToStart);

    TuningFork_ErrorCode StartRecordingLoadingTime(const LoadingTimeMetadata& metadata,
                                                   const ProtobufSerialization& annotation,
...
private:
    ...
    TuningFork_ErrorCode TickNanos(MetricId compound_id, TimePoint t, MetricData** ppdata);
    ...
    TuningFork_ErrorCode TraceNanos(MetricId compound_id, Duration dt, MetricData** ppdata);
    ...
    TuningFork_ErrorCode SerializedAnnotationToAnnotationId(const SerializedAnnotation& ser,
    ...
    TuningFork_ErrorCode AnnotationIdToSerializedAnnotation(AnnotationId id,
                                                            SerializedAnnotation& ser) override;

    TuningFork_ErrorCode LoadingTimeMetadataToId(const LoadingTimeMetadataWithGroup& metadata,
                                                 LoadingTimeMetadataId& id);

    TuningFork_ErrorCode MetricIdToLoadingTimeMetadata(MetricId id,
                                                       LoadingTimeMetadataWithGroup& md) override;
    ...
    bool Loading() const {
        return live_loading_events_.size() > 0;
    }
    ...
    void CreateSessionFrameHistograms(Session& session, size_t size,
                                      int max_num_instrumentation_keys,
    ...
    TuningFork_ErrorCode RecordLoadingTime(LoadingHandle handle, ProcessTimeInterval interval);
};
