...
struct MemoryMetric {
    ...
    MemoryMetric(int64_t avail_mem, int64_t oom_score, int64_t proportional_set_size,
    ...
    void UpdateValues(int64_t avail_mem, int64_t oom_score, int64_t proportional_set_size,
                      Duration time_since_process_start) {
        avail_mem_ = avail_mem;
        oom_score_ = oom_score;
        proportional_set_size_ = proportional_set_size;
...
struct MemoryMetricData : public MetricData {
    MemoryMetricData(MetricId metric_id) : MetricData(MetricType()), metric_id_(metric_id) {}
    MetricId metric_id_;
    std::vector<MemoryMetric> data_;
    int cyclical_buffer_location = 0;

    void Record(IMemInfoProvider* mem_info_provider, Duration time_since_process_start) {
        ...
        if (data_.size() < kBufferSize) {
            ...
            data_[cyclical_buffer_location].UpdateValues(mem_info_provider->GetAvailMem(),
                                                         mem_info_provider->GetMemInfoOomScore(),
