...
public class InputConnection extends BaseInputConnection implements View.OnKeyListener {
    ...
    public InputConnection(Context ctx, View targetView, Settings settings) {
        ...
        WindowCompat.setDecorFitsSystemWindows(
                ((Activity) targetView.getContext()).getWindow(), false);
    ...
    public boolean setComposingText(CharSequence text, int newCursorPosition) {
        Log.d(TAG,
                String.format(
                        "setComposingText='%s' newCursorPosition=%d", text, newCursorPosition));
        if (text == null) {
            return false;
        }
    ...
    private boolean processKeyEvent(KeyEvent event) {
        ...
        Log.d(TAG,
                String.format(
                        "processKeyEvent(key=%d) text=%s", keyCode, this.mEditable.toString()));
    ...
    private final void stateUpdated() {
        ...
        // We always propagate state change events because unfortunately keyboard visibility
        // functions are unreliable, and text editor logic should not depend on them.
        if (listener != null) {
